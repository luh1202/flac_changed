...
!print *,'(nx-1)/2=',(nx)*0.5
!t = 0
!nelem_inject = dike_depth/(ABS(rzbo/(nz-1)))
!print *, 'nelem_inject=',nelem_inject
...
!!print *, 'nelem_inject=',nelem_inject
!     if( (i .eq. (nx-1)/2) .and. (j .le. nelem_inject+1) ) then !nx/2 = (121/2) = 60 (round off by int), nx is the node number in x direction, however,; i loop from 1 to 40, which is the number of element in y direction.
!
!
!!       if ((temp_ave.le.600) .and. (time .lt. time_max*0.5).and.((zcord_top-zcord_ave) .le. 7e3) .and.(fa * SIN(2 * pi/fb * time) + fc).ge.0.6) then
!!       mark(kk)%phase = 3
!!       else if ((temp_ave.le.600) .and. (time .ge. time_max*0.5) .and.((zcord_top-zcord_ave) .le. 7e3) .and.(fa * SIN(2 * pi/fb * time) + fc).ge.0.6) then
!!       mark(kk)%phase = 4
!       !if ((temp_ave.le.600) .and. (time .lt. time_max*0.5).and.((zcord_top-zcord_ave) .le. 7e3)) then
!       !mark(kk)%phase = 3
!       !else if ((temp_ave.le.600) .and. (time .ge. time_max*0.5) .and.((zcord_top-zcord_ave) .le. 7e3)) then
!       !mark(kk)%phase = 3
!
...
!     end if
!
!     if( (i .eq. (nx-1)/2) .and. (j .le. nelem_inject+1) ) then
!
!       !if( (i .eq. (nx-1)/2) .and. (j .le. 10) .and. ( iph .eq. 5) )  then
!       !mark(kk)%phase = 5
!
...
!
!    iph = iphase(j,i)
!    if ((i .le. (nx-1)/2) .and. (j .le. 20) .and. (aps(j,i) .ge. 0.1) .and. ( iph .eq. 3) )  then
...
! new starts
!zcord_ave = 0.25 * (cord(j,i,2) + cord(j+1,i,2) + cord(j,i+1,2) + cord(j+1,i+1,2))
!if( (i .eq. (nx-1)/2) .and. (j .le. 4+1) ) then
!mark(kk)%phase = 1
!else if ((i .eq. (nx-1)/2) .and. (j .gt. 4+1) .and. (j .le. 14+1)) then
!mark(kk)%phase = 3
!else if ((i .eq. (nx-1)/2) .and. (j .gt. 14+1)) then
!mark(kk)%phase = 2
!end if
!new ends
!if( (i .gt. (nx-1)/2) .and. (j .le. nelem_inject+1) ) then
!if ((temp_ave.le.600) .and. (time .lt. time_max*0.1)) then
!mark(kk)%phase = 1
!else if ((temp_ave.le.600) .and. (time .ge. time_max*0.1) .and. (time .lt. time_max*0.2)) then
!mark(kk)%phase = 1
